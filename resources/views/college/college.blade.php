    ...
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                <a href="{{ route('add-college') }}" type="button" class="btn btn-outline-success me-1 mb-1 btn-sm"
                ...
                <tbody>
                    @foreach ($data as $key => $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('students-by-collage', ['id' => $item->id, 'name' => $item->college_name]) }}"
                                    target="_blank" rel="noopener noreferrer">{{ $item->college_name }}</a>
                            </td>
                            <td>{{ $item->location }}</td>
                            <td>
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu fs-sm" aria-labelledby="dropdown-default-outline-primary">
                                        <a class="dropdown-item"
                                            href="{{ route('edit-college', ['id' => $item->id]) }}">Edit</a>
                                        @if ($item->status == 'VISIBLE')
                                            <div class="dropdown-divider"></div>
                                            <a class="dropdown-item bg-danger text-white"
                                                href="{{ route('status-college', ['status' => 'HIDDEN', 'id' => $item->id]) }}">Mark
                                                as Hidden</a>
                                        @else
                                            <div class="dropdown-divider"></div>
                                            <a class="dropdown-item bg-success text-white"
                                                href="{{ route('status-college', ['status' => 'VISIBLE', 'id' => $item->id]) }}">Mark
                                                as Visible</a>
                                        @endif
